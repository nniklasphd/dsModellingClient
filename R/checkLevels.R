...
#'
checkLevels <- function(form, variables, datasources){
  temp <- variables
  formula <- form
  ...
  for(i in 1:length(temp)){
    ...
    if(length(unique(unlist(aa))) == 1 & unique(unlist(aa)) == "factor"){
      ...
      if(length(qq) > 1){ varName <- temp[[i]] }else{ varName <- qq[1] }
      ...
      for(j in 1:length(datasources)){
        ...
        aa <- datashield.aggregate(datasources[j], cally)
        xx[[j]] <- aa[[1]]
        records <- append(records, length(aa[[1]]))
      }
