...
mod <- ds.glm(formula=D$DIS_DIAB~D$PM_BMI_CONTINUOUS+D$LAB_HDL+D$GENDER,family=quote(binomial))
# TODO do more than a smoke test

context("dsbaseclient::ds.glm()  run the above GLM model with an intercept (eg. intercept = 1)")
mod <- ds.glm(formula=D$DIS_DIAB~1+D$PM_BMI_CONTINUOUS+D$LAB_HDL+D$GENDER,family=quote(binomial))
# TODO do more than a smoke test
